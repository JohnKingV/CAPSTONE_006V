...
@router.get("/_ping", summary="Ping M̩dicos")
def ping():
    return {"ok": True}

@router.get("", response_model=List[schemas.MedicoOut], summary="Listar M̩dicos")
def listar_medicos(db: Session = Depends(get_db)):
    return db.query(models.Medico).order_by(models.Medico.id.desc()).all()

@router.post("", response_model=schemas.MedicoOut, status_code=status.HTTP_201_CREATED, summary="Crear M̩dico")
...
@router.get("/{medico_id}", response_model=schemas.MedicoOut, summary="Obtener M̩dico")
...
@router.patch("/{medico_id}", response_model=schemas.MedicoOut, summary="Actualizar M̩dico")
...
@router.delete("/{medico_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Eliminar M̩dico")
