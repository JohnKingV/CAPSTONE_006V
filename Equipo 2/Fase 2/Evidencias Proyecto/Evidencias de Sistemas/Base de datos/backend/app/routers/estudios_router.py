...
@router.get(
    "",
    response_model=List[schemas.EstudioOut],
    summary="Listar Estudios",
)
def listar_estudios(db: Session = Depends(get_db)):
    estudios = (
        db.query(models.Estudio)
        .options(joinedload(models.Estudio.imagenes))  # 👈 carga imágenes
        .order_by(models.Estudio.id.desc())
        .all()
    )
    return estudios
...
@router.post(
    "",
    response_model=schemas.EstudioOut,
...
def crear_estudio(payload: schemas.EstudioCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Conflicto de datos (FK/unicidad/NOT NULL)",
        ) from e
    db.refresh(est)
    return est
...
def obtener_estudio(estudio_id: int, db: Session = Depends(get_db)):
    est = (
        db.query(models.Estudio)
        .options(joinedload(models.Estudio.imagenes))  # 👈 carga imágenes
        .filter(models.Estudio.id == estudio_id)
        .first()
    )
    if not est:
